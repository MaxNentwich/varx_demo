...
h(1)=subplot(1,4,1); 
imagesc(sqrt(Cmat)); title(['True' newline 'connectivity'])
...
imagesc(R);  title(['VARX' newline 'estimate R']); 
xlabel('Brain areas'); 
colormap('hot')
axis square
...
title(['Model' newline 'connectivity'])
axis square
...
imagesc(SC); title(['Inverse' newline 'covariance'])
