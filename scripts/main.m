...
stim_features = {'saccades', 'fixations'};
str = {'saccades', 'fixations'};
...
for pat = 1:length(patients)
    ...
    for vid=1:length(videos)
        ...
        load([data_dir patients(pat).name '/Stimuli/' video], stim_features{:}, 'frame_saccade', 'frame_fixation', 'pupil');
        ...
        if ~strcmp(vid_name_legacy, 'Resting_fixation') 
            ...
            idx_vid = cellfun(@(C) contains(C, vid_name_legacy), saccade_features_pat.pre_saccade_file);
            ...
            fixation_novelty = saccade_features_vid.distance;
            ...
            fixations(fixations == 1) = fixation_novelty;
        ...
        if strcmp(vid_name_legacy, 'Resting_fixation')
            lfp = lfp(1:fs*300, :);
            hfa = hfa(1:fs*300, :);
            saccades = saccades(1:fs*300);
            fixations = fixations(1:fs*300);
            pupil = pupil(1:fs*300);
            save(sprintf('%s/%s.mat', data_pat_dir, vid_name_legacy), ...
                'lfp', 'hfa', 'saccades', 'fixations', 'pupil', 'fs');
        elseif strcmp(vid_name_legacy, 'Inscapes')
            audio_env = audio_ds';
            save(sprintf('%s/%s.mat', data_pat_dir, vid_name_legacy), ...
                'lfp', 'hfa', 'audio_env',  'saccades', 'fixations', 'pupil', 'fs');
        else 
            audio_env = audio_ds';
            film_cuts = scenes_vec';
            save(sprintf('%s/%s.mat', data_pat_dir, vid_name_legacy), ...
                'lfp', 'hfa', 'film_cuts', 'audio_env',  'saccades', 'fixations', 'pupil', 'fs');
