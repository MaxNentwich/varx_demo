...
def set_ticks_truncate_axes(xt=None, yt=None, left=True, bottom=True):
    ...
    if xt is not None:
        if xt[1] is None:
            plt.xticks(xt[0],xt[0])
        else:
            plt.xticks(xt[0],xt[1])

    if yt is not None:
        if yt[1] is None:
            plt.yticks(yt[0],yt[0])
        else:
...
def plot_MMP(data, annot_file=None, save_file=None, minmax=None, thresh=None, cmap='inferno', alpha=1, add_border=False, bp=3, title=None):
    """
    ...
    if annot_file is None:
        annot_file = subjects_dir + '/fsaverage/label/lh.HCP-MMP1.annot'
    ...
    if len(names)-len(data)==1:
        ...
        if 'aparc.annot' in annot_file:
            
            mmp_id = np.arange(len(names))
            mmp_parcels = pd.DataFrame({'parcel_id':mmp_id, 'parcel_name':names})
            
            vtx_data = np.array([np.nan] * len(mmp_labels))
            
            for l in range(len(mmp_parcels)):            
                idx_parcel = mmp_labels == mmp_parcels.parcel_id[l]
                vtx_data[idx_parcel] = data[l]
